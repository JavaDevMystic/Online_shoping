...
public class ProductController {
    ...
    public BaseResponse<ProductDto> get(@PathVariable long id) {
        ...
        if (dto !=null){
            return new  BaseResponse<>(dto,200);
        }else {
            ErrorData errorData = new ErrorData("Brat bunday data bizda yo'q ekan. Uzur! Adashgan bo'lishingiz mumkun bir eslab ko'ring","PRODUCT_NOT_FOUND");
            return new BaseResponse<>(errorData,404);
        }
    }

    @PostMapping("/create")
    @PreAuthorize("hasRole('ADMIN')")
    public BaseResponse<Long> create(@Valid  @RequestBody ProductCreateDto dto) {
        Long newId = productService.create(dto);
        if (newId==-2){
            return new BaseResponse<>(new ErrorData("Bro bunday brand yo'q ekan. Kechirasiz endi qo'sholmaysiz."));
        }
        else if (newId != null) {
